...
class MyFaiss:
  def __init__(self, root_database: str, bin_file: str, json_path: str):    
    self.index = self.load_np_file(bin_file)
  ...
  def load_np_file(self, bin_dir: str):
    print("Loading index...")
    d = 512
    index = faiss.IndexFlatL2(d)
    # files = [f for f in os.listdir(bin_dir) if f.endswith(".npy")]
    # files.sort()
    # for file in files:
    for file in os.listdir(bin_dir):
        if file.endswith(".npy"):
            ...
            file_npy = np.load(file)
              # change to float32
            file_npy = file_npy.astype(np.float32)
            index.add(file_npy)
...
def main():
  ...
  json_path = 'dict_/keyframes_id.json'

  cosine_faiss = MyFaiss('./database', bin_file, json_path)

  ##### IMAGE SEARCH #####
  i_scores, _, infos_query, i_image_paths = cosine_faiss.image_search(id_query=0, k=1)
  ...
  scores, _, infos_query, image_paths = cosine_faiss.text_search(text, k=1)
  # cosine_faiss.write_csv(infos_query, des_path='/content/submit.csv')
