...
PATH = 'dataset/map-keyframes-b1/map-keyframes/'
...
def write_csv(id2img_fps, mode_write_csv, selected_image, id, des_path):
    ...
    map_df = pd.read_csv(PATH + video_name_selected.replace('.mp4', '.csv'), index_col=0)
    id_frame_selected = int(id_frame_selected)
    if id_frame_selected in map_df.index:
      id_frame_selected = str(int(map_df.loc[id_frame_selected]['frame_idx']))
    # print(f"Selected frame: {video_name_selected} - {id_frame_selected}")
